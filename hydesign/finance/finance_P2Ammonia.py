...
class finance_P2Ammonia(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('m_NH3_t',
                       desc = "Produced Ammonia",
                       units = 'kg',
                       shape=[self.life_h])
        
        self.add_input('Q_t',
                       desc = "Heat production",
                       units = 'MW',
                       shape=[self.life_h])
        
        self.add_input('m_NH3_offtake_t',
                       desc = "Selled Ammonia",
                       units = 'kg',
                       shape=[self.life_h])
        ...
        self.add_input('CAPEX_NH3',
                       desc = "CAPEX Ammonia plant")
        self.add_input("OPEX_NH3",
                       desc = "OPEX Ammonia plant")
        ...
        self.add_input('NH3_WACC',
                       desc = "After tax WACC for NH3 plant")
        ...
        self.add_output('annual_NH3',
                        desc = "Annual NH3 production")
        
        self.add_output('annual_Q',
                        desc = "Annual Heat production")
        ...
        self.add_output('break_even_NH3_price',
                        desc='price of Ammonia that results in NPV=0 with the given hybrid power plant configuration and operation',
                        val=0)
    ...
    def compute(self, inputs, outputs):
        ...
        revenues = calculate_revenues_P2Ammonia(price_H2, price_t, price_NH3, df)
        CAPEX = inputs['CAPEX_w'] + inputs['CAPEX_s'] + \
            inputs['CAPEX_b'] + inputs['CAPEX_el'] + inputs['CAPEX_ptg'] + inputs['CAPEX_NH3']
        OPEX = inputs['OPEX_w'] + inputs['OPEX_s'] + \
            inputs['OPEX_b'] + inputs['OPEX_el'] + inputs['OPEX_ptg'] + \
            inputs['water_consumption_cost'] + inputs['OPEX_NH3']
        ...
        # Discount rates
        WACC_after_tax = calculate_WACC_P2Ammonia(
            inputs['CAPEX_w'],
            inputs['CAPEX_s'],
            inputs['CAPEX_b'],
            inputs['CAPEX_el'],
        ...
        CAPEX_NH3 = inputs['CAPEX_NH3']
        OPEX_NH3 = inputs['OPEX_NH3']

        hpp_discount_factor_H2 = inputs['ptg_WACC']
        hpp_discount_factor_NH3 = inputs['NH3_WACC']

        OPEX_ptg_el = LCOE*np.sum(inputs['P_ptg_t']) #operational cost for the electricity consumed to produce hydrogen
        OPEX_NH3_el = LCOE * np.sum(inputs['P_HB_t'] + inputs['P_ASU_t'])
        level_costs_H2 = np.sum(OPEX_ptg / (1 + hpp_discount_factor_H2)**iy) + OPEX_ptg_el + CAPEX_ptg
        level_costs_NH3 = np.sum((OPEX_NH3 + OPEX_ptg) / (1 + hpp_discount_factor_NH3)**iy) + OPEX_NH3_el +  OPEX_ptg_el + CAPEX_NH3 + CAPEX_ptg
        level_costs_NH3_wo_elec = np.sum((OPEX_NH3 + OPEX_ptg) / (1 + hpp_discount_factor_NH3)**iy) +  CAPEX_NH3 + CAPEX_ptg
        ...
        if level_ANH3P > 0:
            outputs['LCONH3'] = level_costs_NH3/ (level_ANH3P) # in Euro/kg
        else:
            outputs['LCONH3'] = 1e6
        ...
        break_even_PPA_price = np.maximum(
            0, 
            calculate_break_even_PPA_price_P2Ammonia(
                df = df, 
                CAPEX = CAPEX_eq, 
                OPEX = OPEX, 
                tax_rate = inputs['tax_rate'],
        ...
        break_even_NH3_price = calculate_break_even_NH3_price(
            df = df, 
            CAPEX = CAPEX_eq, 
            OPEX = OPEX, 
            tax_rate = inputs['tax_rate'], 
...
def calculate_WACC_P2Ammonia(
    ...
    WACC_after_tax = \
        ( CAPEX_w * wind_WACC + \
          ...
          CAPEX_ptg * ptg_WACC +\
          CAPEX_el * (wind_WACC + solar_WACC + battery_WACC + ptg_WACC + NH3_WACC)/5 ) / \
        ( CAPEX_w + CAPEX_s + CAPEX_b + CAPEX_el + CAPEX_ptg + CAPEX_NH3)
    return WACC_after_tax


def calculate_revenues_P2Ammonia(price_H2, price_el, price_NH3, df):
    df['revenue'] = df['hpp_t'] * np.broadcast_to(price_el, df['hpp_t'].shape) + df['m_H2_offtake_t'] * price_H2  + df['m_NH3_offtake_t']* price_NH3 - df['penalty_t']
    
    return df.groupby('i_year').revenue.mean()*365*24
    

def calculate_break_even_PPA_price_P2Ammonia(df, CAPEX, OPEX, tax_rate, discount_rate, price_H2, price_NH3,
                                       depreciation_yr, depreciation, DEVEX, inflation_index):
    def fun(price_el):
        revenues = calculate_revenues_P2Ammonia(price_H2, price_el, price_NH3, df)
...
def calculate_break_even_H2_price(df, CAPEX, OPEX, tax_rate, discount_rate, price_el, price_NH3,
                                  depreciation_yr, depreciation, DEVEX, inflation_index):
    def fun(price_H2):
        revenues = calculate_revenues_P2Ammonia(price_H2, price_el, price_NH3, df)
        NPV, _ = calculate_NPV_IRR(
...
def calculate_break_even_NH3_price(df, CAPEX, OPEX, tax_rate, discount_rate, price_el, price_H2,
                                   depreciation_yr, depreciation, DEVEX, inflation_index):
    def fun(price_NH3):
        revenues = calculate_revenues_P2Ammonia(price_H2, price_el, price_NH3, df)
        NPV, _ = calculate_NPV_IRR(
