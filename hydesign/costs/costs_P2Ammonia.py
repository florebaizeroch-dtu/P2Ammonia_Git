...
class ptg_cost(om.ExplicitComponent):
    ...
    def __init__(self,
                 ...
                 water_consumption,
                 H2_storage_capex_cost,
                 H2_storage_opex_cost,
                 N_time,
                 life_y = 25,
                 intervals_per_hour = 1,
                 ):
        ...
        self.storage_capex_cost = H2_storage_capex_cost
        self.storage_opex_cost = H2_storage_opex_cost
        self.N_time = N_time
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['CAPEX_ptg'] = ptg_MW * 1e3 * (electrolyzer_capex_cost) + storage_capex_cost * HSS_kg  
        outputs['OPEX_ptg'] = ptg_MW * 1e3 * (electrolyzer_opex_cost) + storage_opex_cost * HSS_kg
...
class NH3_cost(om.ExplicitComponent):
    """Ammonia production cost model is used to calculate the overall NH3 plant cost. The cost model includes cost of Air Separation Units to produce N2,
    ...
    def setup(self):
        ...
        self.add_input('NH3_tpd',
                        desc = "Installed capacity of the Haber-Bosch plant"
                        )
        self.add_input('NH3SS_kg',
                        desc = "Installed capacity of Ammonia storage",
                        units = 'kg')

        
        #Creating outputs:
        self.add_output('CAPEX_NH3',
                        desc = "CAPEX power to gas")
        self.add_output('OPEX_NH3',
                        desc = "OPEX power to gas") 

    
    def compute(self, inputs, outputs):

        NH3_tpd = inputs['NH3_tpd'][0]
        NH3SS_kg = inputs['NH3SS_kg'][0]

        NH3_HB_capex_cost = self.NH3_HB_capex_cost
        NH3_HB_opex_cost = self.NH3_HB_opex_cost 
        ASU_capex_cost = self.ASU_capex_cost
        NH3_storage_capex_cost = self.NH3_storage_capex_cost

        outputs['CAPEX_NH3'] = NH3_tpd * (ASU_capex_cost + NH3_HB_capex_cost) + NH3_storage_capex_cost * NH3SS_kg  
        outputs['OPEX_NH3'] = NH3_tpd * (NH3_HB_opex_cost)
